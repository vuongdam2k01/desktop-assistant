...
const fullfsyncMode = process.env.SQLITE_FULLFSYNC === '1' ? 'fullfsync_on' : 'fullfsync_off';
const synchronousMode = process.env.SQLITE_SYNCHRONOUS || 'NORMAL';
...
console.log(`\n======================================================================`);
console.log(`🧪 TESTING CRASH POINT ${point} [Mode: ${fullfsyncMode}, sync: ${synchronousMode}]: ${description}`);
console.log(`======================================================================`);

const evidenceDir = path.join(__dirname, '../evidence');
if (!fs.existsSync(evidenceDir)) fs.mkdirSync(evidenceDir, { recursive: true });

const dbPath = path.join(evidenceDir, `crash_pt${point}_${fullfsyncMode}.db`);
const notionStatePath = path.join(evidenceDir, `notion_pt${point}_${fullfsyncMode}.json`);
const logFilePath = path.join(evidenceDir, `crash-point-${point}-${fullfsyncMode}.log`);
...
log(`[RUNNER] Launching worker with CRASH_POINT=${point}, FULLFSYNC=${process.env.SQLITE_FULLFSYNC || '0'}, SYNC=${synchronousMode}...`);
...
const workerEnv = {
  ...
  SQLITE_FULLFSYNC: process.env.SQLITE_FULLFSYNC || '0',
  SQLITE_SYNCHRONOUS: synchronousMode
};
...
// On macOS (POSIX), process.kill(pid, 'SIGKILL') sets proc.signal = 'SIGKILL'
...
const db = createDatabase(dbPath, {
  fullfsync: process.env.SQLITE_FULLFSYNC === '1',
...
if (point === '1') {
  ...
  assert(errorRecord !== undefined, 'Error record must be present in ledger');
  const topCard = recoveryResult.cardQueue[0];
  ...
  assert(resultRecord !== undefined, 'Tool result record must be appended by reconciliation');
