...
app.whenReady().then(() => {
  try {
    ...
    insertStmt.run('corr_packaged_001', JSON.stringify({ action: 'packaged_native_check', timestamp: new Date().toISOString() }));

    const row = db.prepare('SELECT * FROM test_ledger WHERE correlation_id = ?').get('corr_packaged_001');
    db.close();
