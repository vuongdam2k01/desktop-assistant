...
export function evaluateTargetScopePredicate(
  ...
  if (pred.ancestor_ids !== undefined) {
    if ("contains" in pred.ancestor_ids && pred.ancestor_ids.contains) {
      if (!ancestorIds.includes(pred.ancestor_ids.contains)) return false;
    }
    if ("contains_any" in pred.ancestor_ids && Array.isArray(pred.ancestor_ids.contains_any)) {
      const match = pred.ancestor_ids.contains_any.some((id: string) => ancestorIds.includes(id));
