...
- Q-1: What are the signing and notarisation steps on macOS, and what do they cost? — source: `spikes/SP-16-signing-update/REPORT.md#5-chua-tra-loi-duoc-vi-sao` — deferred by decision; recorded UNVERIFIED — blocking: any macOS release — decided by: a dedicated macOS spike
- Q-2: Which commercial certificate is purchased, and under a company or an individual identity? — source: `spikes/SP-16-signing-update/REPORT.md#0-ket-luan` — an individual certificate still triggers a reputation warning for early beta users — blocking: the first closed beta, with a procurement lead time — decided by: product owner
