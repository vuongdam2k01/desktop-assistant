...
It was amended at `1.1.0` by `req-003-notion-compensation`, the first amendment made by a connector rather than
by the framework, which added `side_effects` and `compensation.arguments_source` from measuring one platform's
write operations end to end.

This version, `1.2.0`, is the amendment the coordinator needs. It adds two optional objects whose shape is owned
by `connector/contracts/coordination-declaration@0.1.0`: per tool, which of its own arguments name the objects it
may change; per connector, how that platform's identifiers are spelled and what concurrency and pacing it
tolerates. Neither adds a mechanism to this contract — both are declarations the coordinator reads so that it can
hold an object exclusively while a call's before state is recorded, without knowing anything about the platform
the object lives on. The measurement that makes them necessary is
`spikes/SP-15-concurrency/REPORT.md` §1 Q2: without that exclusive span, a second job records a before state that
is already false, and compensating it destroys the first job's completed work.

One behavioural consequence comes with the addition, and it is stated here rather than buried in Migration: a
tool whose `direction` is `write` must now declare `tool_coordination`, and a manifest whose write tool does not
is refused whole, exactly as a write tool that declares neither a compensation nor irreversibility is refused
today. The reason is the same in both cases — a partially loaded connector would offer a write the product cannot
keep its promises about.
...
The file is the manifest's shape at `1.2.0`, and it is what a connector author's manifest is checked against at
load. This version's addition is in it structurally rather than as advice: `tool_coordination` is required on
every write tool, so a manifest whose write declares no resources is refused whole — which is the point, because
a write whose resources nobody declared is a write the coordinator cannot protect. The optional
`connector_coordination` carries this platform's own pacing, concurrency and identifier spelling, each bounded by
what was measured.

What the file cannot express is stated in this document instead and is enforced in the same registration pass:
that a declared argument path exists in that tool's own parameters, that a declared resource type is used
consistently across the connector, that a tool's `capability` is one the manifest declares, and that
`snapshot.read_operation` and `compensation.tool` name tools this manifest declares. A declaration that names too
little satisfies the file and is the most dangerous thing this contract admits — the coordinator then locks
nothing while believing it has locked something — which is why the resource declarations are reviewed by reading
...
import type { ToolCoordination, ConnectorCoordination } from "connector/contracts/coordination-declaration@0.1.0";

interface ConnectorManifest {
  schema_version: "1.0" | "1.1" | "1.2"; // major.minor of THIS contract, not of the connector
  ...
  connector_coordination?: ConnectorCoordination;  // added at 1.2.0; absent means the product's defaults
...
interface ToolDeclaration {
  ...
  tool_coordination?: ToolCoordination;  // write only; added at 1.2.0; REQUIRED when direction is "write"
...
this version's coordination members. It is not restated here: a manifest that satisfies that file and the
whole-manifest rules under *Semantics* is a manifest this build loads, and a second copy of the shape in prose
would be the thing that drifts from it.
...
`exclude_computed` being required inside a `snapshot`, `tool_coordination` being required on every write tool with
at least one resource in it, and the bounds on every figure a connector may state about its own pacing and
concurrency. What it cannot express, and what *Semantics* and the table of refusals below hold instead: that a
declared argument path exists in that tool's own parameters, that a declared resource type means the same thing
across the connector's tools, that a tool's `capability` is one the manifest declares, and that
`snapshot.read_operation` and `compensation.tool` name tools this manifest declares. Each is a relation between
members rather than a property of one, so a manifest can satisfy the file and still be refused at registration.
...
- **`tool_coordination` is required on every write tool.** It names, for each object the tool may change, the
  argument path carrying that object's identifier and the connector's own word for its type. The coordinator
  derives a resource key from the same argument the platform call will address and holds that object exclusively
  from before the before state is read until the result record is durable — which is what makes a recorded
  before state true when a compensating action is later built from it, VERIFIED
  (`spikes/SP-15-concurrency/REPORT.md` §1 Q2). A path the tool's own `parameters` does not contain is refused,
  and a read tool carrying `tool_coordination` is refused as a contradiction, in the same way a read tool
  carrying `snapshot` is. There is deliberately no way for a call to supply its own key: a key travelling beside
  the arguments would be a model-produced string deciding what is protected, so the object held and the object
  changed could differ.
- **`connector_coordination` is optional and describes the platform, not the product's appetite.** Absent, the
  product's defaults apply — four concurrent jobs per account of this connector with one slot reserved for a job
  the user started, a 15-second wait for a held object, and this manifest's existing `rate_policy` for pacing. A
  declared `concurrency`, `rate` or `wait_limit_ms` outside the accepted range is clamped and the clamp is
  reported to the author rather than the manifest being refused, because a pacing opinion should not cost the
  product a platform. `identifier_normalisation` may only reduce spellings of one identifier to one key; a rule
  that could map one object to two keys is refused, since two keys for one object is the absence of exclusivity
  wearing its name. `class_weights` and `background_floor` are published as provisional defaults: the benchmark
  that measured fair dispatch ran plain round-robin, and both numbers are recommendations nobody has run —
  `verification.md` of `req-015-concurrency-coordinator` records them as unverified.
...
| `COORDINATION_MISSING` | A write tool carries no `tool_coordination` | Callee, at load | The whole connector fails to load; the author sees which tool and why. A write nobody can hold exclusively is a write whose recorded prior state may be false |
| `COORDINATION_ON_READ_TOOL` | A read tool declares `tool_coordination` | Callee, at load | As above; a read takes no exclusive access, so the declaration states something that cannot be true |
| `COORDINATION_PATH_UNKNOWN` | A declared resource `path` names an argument the tool's `parameters` does not contain | Callee, at load | As above, naming the offending path |
| `NORMALISATION_UNSAFE` | A declared `identifier_normalisation` could map one object to more than one key | Callee, at load | As above; the rule is named, because the alternative is exclusivity the product believes it has and does not |
| `COORDINATION_POLICY_CLAMPED` | A declared `concurrency`, `rate` or `wait_limit_ms` lies outside the accepted range | Callee, at load | Nothing: the value is clamped and the clamp is reported to the connector's author |
...
before. Version `1.1.0` was MINOR on three counts: `side_effects` is a new optional field, `arguments_source` is a new
optional field whose absence means what `1.0.0` meant, and `COMPENSATION_WITHOUT_SNAPSHOT` refuses a strict
subset of what it refused before.

Version `1.2.0` is MINOR on the structural test — both additions are optional objects, and every `1.1.0` manifest
still validates — and it is the borderline case worth naming rather than glossing. `tool_coordination` is
required of a write tool, so an existing manifest that declares write tools yields no tools until it is updated.
That is a narrowing of what a valid manifest produces rather than of what validates, which is how `1.0.0` already
treats a write tool declaring neither a compensation nor irreversibility; and because every manifest ships inside
the build that reads it, the population that must be updated is enumerable and is enumerated in Migration. Were
manifests ever to arrive from outside the build — the reserved point in `req-019-connector-framework`'s model —
this same addition would be MAJOR, and the reserved point is where that is reconsidered.
...
does not know `side_effects` or `tool_coordination` does not ignore it, it refuses the manifest. That is the correct behaviour while
...
the additions of both amendments: a creation whose compensating action is built from the platform's response, a
write that emits a notification nothing withdraws, and — new at `1.2.0` — the resource each write tool may change
together with the platform's identifier spelling and its concurrency tolerance. The full tool set, with the snapshot and compensation of each, is
...
```json
{
  ...
  "tools": [
    ...
    },
    {
      ...
      "tool_coordination": {
        "resources": [
          { "type": "database", "path": "database_id" }
        ]
      }
    },
    {
...
**Rejected** — a write that promises a withdrawal it cannot perform and names no object it changes:
...
The manifest is refused four times over, and the whole connector with it: `SIDE_EFFECT_WITHDRAWAL_UNDECLARED`,
because an effect declared recallable names no tool that recalls it; `TOOL_REFERENCE_UNKNOWN` twice, because
neither the read operation its snapshot names nor the tool its compensation names is declared here; and
`COORDINATION_MISSING`, because a write tool declares no object it may change. The last one is the addition of
`1.2.0` doing its work at the only moment it can: a tool that reaches an agent without naming its target can
still be locked by nothing, and the product would discover that only when a second job corrupted the first's
recorded prior state. The refusal belongs at load, where the connector's author is looking, rather than at the
first call, where a user is.

## Migration

**From `1.1.0` to `1.2.0`.** Structurally additive — every `1.1.0` manifest still validates — with one
behavioural requirement that an existing manifest must meet before its write tools are offered again.

1. **`tools[].tool_coordination` is new and is required of every write tool.** For each object the tool may
   change, name the argument path that carries the object's identifier and the connector's own word for its
   type. Name the path the platform call itself addresses; a field invented to carry a key is refused, because a
   key produced alongside the arguments could name a different object than the call changes.
2. **`connector_coordination` is new and optional.** Absent, the product's defaults apply: four concurrent jobs
   per account with one reserved for a job the user started, a 15-second wait for a held object, and this
   manifest's existing `rate_policy` for pacing. Declare `identifier_normalisation` only when the platform
   accepts an identifier in more than one spelling — the measured platform does — and declare `concurrency` only
   from a measurement of that platform.
3. **`schema_version` accepts `"1.2"`.** A manifest using either new object must declare `"1.2"`, since a build
   that predates them refuses unknown fields rather than ignoring them.

**Consumers to update.** `connector` gains the coordinator that reads both objects, specified in
`req-015-concurrency-coordinator`; `job` reads nothing new but classifies a new family of refusal codes;
`ledger`, `agent`, `approval`, `undo`, `backend`, `app` and `uix` read nothing that changed shape or meaning.
**Manifests to update** are enumerable because every manifest ships inside the build: the connector of
`req-003-notion-compensation` declares write tools and must declare their resources; the connector of
`req-014-byo-oauth-google` is read-only and may stay at `"1.0"`. Until a write tool declares its resources its
manifest yields no tools at all, which is deliberate and is the same treatment `1.0.0` gives a write tool that
declares neither a compensation nor irreversibility.
